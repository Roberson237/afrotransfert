...
const AuthModal = ({ isOpen, onClose, onContinueWithoutSignIn }) => {
  ...
  const handleGoogleAuth = () => {
    ...
    setLoginError('');
    setRegisterError('');
    signIn('google', { callbackUrl: '/page/uploadspace' }).catch(() => {
      setIsLoading(false);
      setLoginError(getFriendlyErrorMessage('Google OAuth configuration error', 'La connexion Google a échoué. Vérifiez votre configuration puis réessayez.'));
    });
  };

  const handleContinueWithoutSignIn = async () => {
    router.push('/page/uploadspace');
  };
                    ...
                    <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="p-6">
                      ...
                      <form
                        onSubmit={async (e) => {
                          ...
                          setRegisterError('');
                          const formData = new FormData(e.currentTarget);
                          const result = await New_User(formData);
                          if (result.success) {
                            router.push('/page/uploadspace');
                          } else {
                      ...
                      >
                        <Tabs.Content value="register" className="space-y-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-2">8+ characters with letters and numbers</p>
                          </div>
